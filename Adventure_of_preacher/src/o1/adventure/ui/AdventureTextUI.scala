 ...
 * File: AdventureTextUI.scala - the Text UI for the game.
 */
...
object AdventureTextUI extends App {
  ...
  /** Runs the game. First, a welcome message is printed, then the player gets the chance to
    ...
    * Game logic is split to few methods that are run in series. */
  private def run() = {
    println(this.game.welcomeMessage)
    while (!this.game.isOver) {
      this.printAreaInfo()
      print(this.game.Interview)
      //print(this.player.battleEnds)
      this.game.decision
      this.playTurn()
    }
